...
class ItemsService {
    ...
    async createOne(data, opts) {
        ...
        const primaryKey = await this.knex.transaction(async (trx) => {
            ...
            if (this.accountability && this.schema.collections[this.collection].accountability !== null) {
                ...
                const activityID = (await trx.insert(activityRecord).into('directus_activity').returning('id'))[0];
                ...
                if (this.schema.collections[this.collection].accountability === 'all') {
                    ...
                    const revisionID = (await trx.insert(revisionRecord).into('directus_revisions').returning('id'))[0];
        ...
        await this.knex.transaction(async (trx) => {
            ...
            if (this.accountability && this.schema.collections[this.collection].accountability !== null) {
                ...
                for (const activityRecord of activityRecords) {
                    const primaryKey = (await trx.insert(activityRecord).into('directus_activity').returning('id'))[0];
                ...
                if (this.schema.collections[this.collection].accountability === 'all') {
                    ...
                    for (let i = 0; i < revisionRecords.length; i++) {
                        const revisionID = (await trx.insert(revisionRecords[i]).into('directus_revisions').returning('id'))[0];
                        if (opts === null || opts === void 0 ? void 0 : opts.onRevisionCreate) {
                            opts.onRevisionCreate(revisionID);
                        }
                        if (i === 0) {
    ...
    async update(data, key) {
        ...
        await this.knex.transaction(async (trx) => {
            ...
            const payloads = (0, utils_1.toArray)(data);
            for (const single of payloads) {
                const payload = (0, lodash_1.clone)(single);
